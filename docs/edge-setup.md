...
> 각 단계의 **`→`** 가 나와야 다음으로 간다.
> **1~8 은 끝났다 (2026-09-03). 9단계 측정도 끝났다 (2026-09-06) —
...
확정하면 `plan.md` §2 에 적는다.

---

## 🛒 10단계 — 사야 할 것 ⬅️ **지금 여기** (2026-09-06 조사)
...
이 카메라가 먹는 전기는 **최대 7.5W** 이고 규격은 **802.3af** 다 (제조사
사양 확인). 고른 인젝터는 15.4W 짜리라 두 배 여유다. 더 비싼 것을 살 이유가 없다.
...
무엇을 사는지는 **10단계**에 있다. 물건이 온 뒤에 할 코드 작업이 이 둘이다.

0. **하드웨어 디코딩이 살아있는지 확인하는 코드.** 지금은 꺼져도 오류가
   안 나고 조용히 느려지기만 한다 (9단계 실측에서 이게 판정을 가르는
   갈림길이었다). T8 에 같이 넣는다.
