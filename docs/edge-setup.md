...
> **다음 순서는 저장소 맨 위 `문서/다음할일.md` 에 있다.**
>
...
확정하면 `문서/plan.md` §2 에 적는다. → **확정했다 (4MP·H.265·2.8mm).** 실시간으로
...
- **카메라용 SD 카드.** 카메라에 영상을 저장하면 "영상 미저장" 개인정보
  예외 조항이 깨진다 (`문서/plan.md` T10). 우리는 판정만 하고 영상을 남기지 않는다.
- **녹화기(NVR).** 박스가 그 일을 한다.
...
그걸로 카메라 해상도·코덱을 확정하고 `문서/plan.md` §2 에 적는다.
...
- ⬜ `문서/plan.md` **T10 — 24시간 무인 운영.** 자동 재시작(systemd), 로그 정리,
...
→ **`문서/박스업데이트.md` 4절로 옮겼다.** 이 문서는 박스를 **처음 만드는** 순서고,
이미 도는 박스를 갱신하는 것과 정리하는 것은 그쪽에 모아 뒀다.

```bash
bash deploy/box_sparse_checkout.sh   # 181개 → 52개, 한 번만
```
