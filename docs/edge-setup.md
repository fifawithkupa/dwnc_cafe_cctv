...
> **12단계(자동 실행)까지 설치됐고 26시간 무인 운영을 통과했다 (2026-09-07).**
> **다음 순서는 저장소 맨 위 `다음할일.md` 에 있다.**
>
...
저장소에는 기획 문서·테스트·지난 실험 결과가 같이 들어 있다. 박스에서는 하나도 안 쓴다.
용량이 문제는 아니다(전체 2MB 남짓). **어느 파일이 실제로 도는 것인지 헷갈리는 게 문제**다.
판정 기록이 쌓이기 시작하면 더 그렇다.

`git pull` 은 그대로 쓰면서 필요한 것만 풀리게 한다. **한 번만 하면 된다.**
...
**필요한 것만 적고 나머지는 다 빼는** 방식이라, 저장소에 무엇이 새로 생기든
박스는 깨끗하게 유지된다.

| 남는 것 | 왜 |
|---|---|
| `engine/` (8) | 판정 본체 |
| `edge/` (16) | 카메라·전송·측정·텔레메트리 |
| `checks/` (10) | 8단계 채점 |
| `install/` (3) | 설치 당일 좌석 확인 |
| `layouts/` (3) | 이 매장의 좌석 배치 |
| `deploy/` (7) | 서비스 파일·설정·SQL |
| `results/angle1_layout/angle_answer.md` (1) | **채점용 정답지. `results/` 에서 박스가 쓰는 건 이 한 장뿐이다** |
| `README.md`·`CLAUDE.md` | 박스에 들어온 사람이 읽을 곳. 둘 합쳐 15KB |

빠지는 것: `docs/` · `tests/` · 지난 실험 결과 53개 · 기획/설치 문서 · `문큐연락/` · `.claude/`
...
- **코드 폴더가 통째로 사라졌다** — `set` 에 `--no-cone` 을 빼먹으면 패턴이
  폴더 이름 취급을 받아 `engine/` `edge/` 가 다 빠진다. 스크립트에는 이미 붙여 뒀고,
  필수 파일이 하나라도 없으면 **스스로 되돌린다.** 손으로 칠 때만 조심하면 된다.
  (2026-09-11 실제로 겪음)
- **박스가 돌려야 할 코드 폴더를 새로 만들었다** — 허용 목록 방식이라 저절로는
  안 내려온다. `deploy/box_sparse_checkout.sh` 의 `PATTERNS` 에 한 줄 추가한다.
  스크립트가 돌 때 **안 내려오는 폴더 목록을 찍어주므로** 보면 안다.
- **채점할 때 정답지가 없다** — `results/` 는 정답지 한 장만 내려온다.
  다른 실험 결과가 필요하면 그때 패턴에 한 줄 추가하거나, 잠깐 `disable` 했다 다시 켠다.
