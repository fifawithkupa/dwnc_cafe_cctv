...
| 파일                                  | 줄수        | 역할                                                                    |
| ----------------------------------- | ---------: | --------------------------------------------------------------------- |
| `engine/seatnow_core.py`                   | 3,194     | **본체**. 아래 2.2의 6개 계층을 한 모듈에 담음                                       |
| `engine/seatnow.py`                        | 596       | CLI 진입점. 인자 파싱 → 샘플링 루프 → JSONL/MP4 출력                                |
| `checks/verify_seatnow.py`                 | 196       | 결과 JSONL을 수동 라벨 정답(`tests/fixtures/test_video_expectations.json`)과 대조 |
| `engine/seatnow_layout.py`                 | 162       | 수동 좌석 레이아웃 JSON 로드/검증/해상도 스케일 (`SeatLayout`)                          |
| `install/calibrate.py`                      | 304       | 클릭 캘리브레이션 도구 (테이블·의자 등록 → `layouts/*.json`)                           |
...
### 2.2 `engine/seatnow_core.py`의 6개 계층
...
`python -m engine.seatnow <영상.mp4>` 한 번 = 아래 흐름.
...
`SeatNowAnalyzer.analyze()` (`engine/seatnow_core.py:1210`). 프레임 1장 → `TableObservation` 리스트.
...
## 6. 기본 로직 ③ — 레이아웃 모드 (`--layout`)
 ...
 `install/calibrate.py`로 등록한 `layouts/*.json`의 존이 **좌석의 정답**이 되는 운영 모드.
...
`engine/seatnow_core.py:1282`, `:1301`에서 `seat_detections = []`로 고정되어 있어서:

1. **판정 규칙 3번(의자→테이블 점유 전파)이 동작하지 않는다.** `occupancy_state_from_evidence()`는 `occupied_chairs` 인자를 `_ = occupied_chairs`(`engine/seatnow_core.py:850`)로 명시적으로 버린다.
 → 의자에만 가방을 둔 손님, 테이블에서 조금 떨어져 앉은 손님을 놓칠 수 있음
2. `**inferred-seat`(테이블이 사람에 완전히 가려진 경우의 구제)가 사실상 죽어 있다** — 조건인 `has_seat_support()`(`engine/seatnow_core.py:1518`)가 항상 `False`
3. **애매한 포즈의 의자 겹침 구제도 죽어 있다** — `seat_support_score()`(`engine/seatnow_core.py:1331`)가 항상 `0.0`
4. **레이아웃 모드에서 캘리브레이션한 의자 존이 소비되지 않는다** — `LayoutZoneTracker`에 빈 의자 리스트를 넘기고(`engine/seatnow_core.py:1259`), `SeatLayout.chair_boxes()` / `chair_assignments()`를 아무도 호출하지 않는다. `install/calibrate.py`로 의자를 찍는 작업이 현재는 결과에 영향을 주지 않음
...
./venv/bin/python -m checks.verify_seatnow <결과.jsonl>   # 수동 라벨 정답과 대조
```
...
> 3. POS 벤더가 임의 설치를 금지·보증무효화하며, 원격 업데이트로 우리 프로세스를 죽일 수 있다. 결제 보안 규정 위반 소지
> 4. 결제 단말 + 영상 처리 결합 = 사고 시 파장 증폭. "사장님이 운영 주체" 3원칙과도 충돌 (POS는 벤더 관리)
> 5. 랜포트 없음, OS 파편화, 파일럿 섭외 난이도 상승
>
> **더 나은 대안**: 매장에 POS 아닌 다른 PC(사무용·사이니지용)가 있으면 그쪽이 훨씬 안전하다.
> **나중에 옵션 B로 검토할 조건**: Intel 6세대 이상 i3급 + 여유 RAM 4GB + 벤더 서면 허용 +
...
| 프레임 큐  | Redis (확장 시)                                                        |
| 엣지     | 파일럿: 중고 미니PC(Intel i3, Quick Sync) / 확장: 라즈베리파이·Jetson Nano         |
| 엣지 런타임 | OpenVINO (FP32/INT8) — `edge/export.py`로 익스포트, `edge/bench.py`로 tick 예산 산출    |
| 카메라 연결 | RTSP / ONVIF                                                        |
...
3. 파일럿 카페 섭외 (데모 자료: `results/v5_*.mp4`)
