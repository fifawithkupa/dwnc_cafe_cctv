...
> **박스 앞에서 그냥 순서대로 치기만 하려면 `docs/edge-setup.md` 를 본다**
> (1~8단계 + 단계마다 막혔을 때). **이 문서는 그 뒤에 읽는 배경 설명이다** —
> 왜 7.5초가 합격선인지, 못 지켰을 때 쓸 속도 카드가 무엇인지가 여기 있다.
...
- **카메라는 안 꽂는다.** 지금 코드는 **녹화 파일 전용**이고, 라이브 스트림
  읽기는 아직 없다 (`문서/plan.md` T8). 카메라를 꽂아도 안 돈다
...
| `python3 --version` | **3.12** (24.04 기본) | 24.04 면 그대로 간다 — 필요한 것이 전부 설치되는 것을 확인했다. **22.04(3.10)면 안 된다** (`contourpy`·`matplotlib` 에 3.10용 파일이 없다). 노트북은 3.11.9 라, 판정이 다르게 나오면 그때 이 차이를 의심한다 |
...
### ②-1. 실측 — 노트북에서 미리 돌려본 결과 (2026-09-03)

박스에 가기 전에 개발 노트북(Core Ultra 5 226V)에서 OpenVINO를 실제로 깔고
같은 영상을 돌렸다. **판정은 파이토치와 완전히 같았다 — 둘 다 70칸, 오답 2.**
정확도를 잃지 않는다는 것은 이제 추측이 아니다.

| | 틱당 추론 |
|---|---:|
| 파이토치 (지금) | 8.2 ~ 9.5초 |
| 파이토치, 코어 2개로 묶음 | 12.2 ~ 13.9초 |
| **OpenVINO** | **1.13 ~ 1.20초** |
| OpenVINO, 코어 2개로 묶음 | 1.29 ~ 1.59초 |
| OpenVINO + 3프레임(①) | 0.79 ~ 0.89초 |
| ⚠️ OpenVINO, **내장 그래픽 없이 CPU만** | 6.0 ~ 7.6초 |
| ⚠️ OpenVINO, CPU만 + 코어 2개 | 9.5 ~ 11.0초 |

### ②-2. 그래서 박스에서 **제일 먼저** 확인할 것

위 표의 위쪽(1초대)과 아래쪽(10초대)을 가른 것은 코어 개수가 아니라
**내장 그래픽을 쓸 수 있느냐**다. OpenVINO는 가만히 두면 알아서 내장
그래픽으로 넘어간다 (노트북에서 확인함 — 실제 실행 장치가 `GPU.0` 이었다).
**HD 530이 OpenVINO에 잡히지 않으면 CPU로만 돌고, 그러면 위 표의 아래쪽이
되고 박스 코어는 노트북보다 2배쯤 느리다.** 합격선을 못 지킨다.

```bash
./venv/bin/python -c "import openvino as ov; print(ov.Core().available_devices)"
```
...
OpenVINO는 처음 한 번 모델을 그 기계에 맞게 컴파일한다. 노트북에서
**첫 틱이 30~83초**였고 두 번째부터 1초대로 떨어졌다. 박스에서는 더 걸릴 수
있다. **첫 줄의 `inference=` 는 버리고 두 번째 줄부터 본다.**

**①과 합치면 13~20초 추정치가 1~4초가 된다** (내장 그래픽이 잡힌다는 전제).
여기서 끝날 가능성이 크다.
...
**측정해봤고 정확도가 오히려 떨어진다** (`문서/plan.md` §2 결정 기록).
...
2. **`문서/plan.md` T8 — 라이브 스트림 읽기.** 지금 코드는 파일 전용이다.
   tick마다 영상의 특정 시각으로 되감는 구조인데 라이브는 되감기가 안 된다.
   **아키텍처 변경이라 시간이 든다**
3. **`문서/plan.md` T10 — 24시간 무인 운영.** 자동 재시작(systemd), 로그 정리,
   **영상 미저장이 기본값**(개인정보)
